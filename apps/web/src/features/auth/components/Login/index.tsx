...
export function Login({ onSuccess }: LoginProps) {
    ...
    // On localhost: auto-redirect to main site for authentication on first visit.
    // After explicit logout, the dbx_explicit_logout flag blocks this redirect
    // so the user stays on the local login page.
    useEffect(() => {
        ...
        if (isLocal && !hasToken && !isExplicitlyLoggedOut) {
            const params = new URLSearchParams();
            params.set('redirect', window.location.origin);
            window.location.href = `https://www.dbxstudio.com/login?${params.toString()}`;
        }
    }, []);
    ...
    // Handle DBX Studio redirect manually
    const handleDbxStudioRedirect = useCallback(() => {
        localStorage.removeItem('dbx_explicit_logout');
        const targetParams = new URLSearchParams();
        targetParams.set('redirect', window.location.origin);
        targetParams.set('force_login', 'true');
        window.open(`https://www.dbxstudio.com/login?${targetParams.toString()}`, '_blank');
    }, []);
        ...
        return (
            <>
                ...
                <div className="auth-buttons">
                    {isLocal ? (
                        <>
                            <div style={{ backgroundColor: '#2d2d2d', padding: '15px', borderRadius: '8px', marginBottom: '15px', fontSize: '14px', color: '#ccc' }}>
                                <strong>Local Development Mode</strong><br />
                                Authentication must happen through the live DBX Studio website.
                            </div>
                            <button
                                className="google-button"
                                onClick={handleDbxStudioRedirect}
                                disabled={isAuthenticating}
                            >
                                <img src="/assets/dbx-logo-white.png" alt="DBX Studio" style={{ width: 20, height: 20, marginRight: 8, filter: 'invert(1)' }} />
                                Sign in via DBX Studio
                            </button>
                        </>
                    ) : (
                        <>
