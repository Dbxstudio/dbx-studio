...
import { SiSqlite, SiMariadb, SiSnowflake, SiClickhouse, SiSupabase, SiAmazonredshift, SiGooglebigquery } from 'react-icons/si'
...
const getDatabaseIcon = (dialect: string, size = 16) => {
    ...
    if (dialectLower === 'postgres' || dialectLower === 'postgresql') {
        ...
        return <SiAmazonredshift size={size} className="db-type-icon redshift" />
    } else if (dialectLower === 'bigquery') {
        return <SiGooglebigquery size={size} className="db-type-icon bigquery" />
    }
